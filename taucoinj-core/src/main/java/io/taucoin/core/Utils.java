...
public class Utils {
    ...
    /*
    * if a object is null,will return ture
    */
    ...
    public static void uint264ToByteStreamLE(BigInteger val, OutputStream stream) throws IOException {
        byte[] bytes = val.toByteArray();
        if (bytes.length > 33) {
            throw new RuntimeException("Input too large to encode into a uint64");
        }
        ...
        stream.write(bytes);
        if (bytes.length < 33) {
            for (int i = 0; i < 33 - bytes.length; i++)
                stream.write(0);
        }
    }
