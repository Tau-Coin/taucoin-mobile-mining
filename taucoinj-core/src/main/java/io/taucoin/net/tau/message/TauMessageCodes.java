...
public enum TauMessageCodes {
    ...
    private static final Map<TauVersion, Map<Integer, TauMessageCodes>> intToTypeMap = new HashMap<>();
    private static final Map<TauVersion, TauMessageCodes[]> versionToValuesMap = new HashMap<>();

    static {

        versionToValuesMap.put(V60, new TauMessageCodes[]{
                STATUS,
                NEW_BLOCK_HASHES,
                TRANSACTIONS,
                GET_BLOCK_HASHES,
        ...
        versionToValuesMap.put(V61, new TauMessageCodes[]{
                STATUS,
                NEW_BLOCK_HASHES,
                TRANSACTIONS,
                GET_BLOCK_HASHES,
        ...
        versionToValuesMap.put(V62, new TauMessageCodes[]{
                STATUS,
                NEW_BLOCK_HASHES,
                TRANSACTIONS,
                GET_BLOCK_HEADERS,
        ...
        for (TauVersion v : TauVersion.values()) {
            Map<Integer, TauMessageCodes> map = new HashMap<>();
            intToTypeMap.put(v, map);
            for (TauMessageCodes code : values(v)) {
                map.put(code.cmd, code);
            }
        }
    }

    private TauMessageCodes(int cmd) {
        this.cmd = cmd;
    }

    public static TauMessageCodes[] values(TauVersion v) {
        return versionToValuesMap.get(v);
    }

    public static TauMessageCodes fromByte(byte i, TauVersion v) {
        Map<Integer, TauMessageCodes> map = intToTypeMap.get(v);
        return map.get((int) i);
    }

    public static boolean inRange(byte code, TauVersion v) {
        TauMessageCodes[] codes = values(v);
